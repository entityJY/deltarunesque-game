...
@export var beats_per_bullet: float = 2.0
## how many beats the spawner runs for
@export var firing_time: float = 4.0

@export var warning_time : float = .4
@export var flash_time : float = 0.1
## time between damages to player
@export var cooldown_timeout = 5
@export var laser_duration : float = .1
@export var laser_width: float = 160
...
func start_spawning():
	...
	while running:
		...
		new_laser.position.y = randf_range(-540, 540)
